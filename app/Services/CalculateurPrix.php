...
class CalculateurPrix
{
     ...
     *
     * @throws \InvalidArgumentException si le prixHT ou le taux est négatif est négatif
     */
    public function calculerAvecTaxe(float $prixHT, float $tauxTaxe): float
    {
        if ($prixHT < 0) {
            throw new \InvalidArgumentException('Le prix hors taxes ne peut pas être négatif.');
        }
        if ($tauxTaxe < 0) {
     ...
     *
     * @throws \InvalidArgumentException si le prix ou la remise est négative
     */
     ...
     *
     * @throws \InvalidArgumentException si le prix ou le seuil est négatif
     */
